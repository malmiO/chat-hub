...
app.config['ALLOWED_EXTENSIONS'] = {'png', 'jpg', 'jpeg', 'gif','aac','mp3','mp4', 'mov', 'avi','pdf','docx','txt','doc','xlsx','xls'}
...
@app.route('/upload-group-document', methods=['POST'])
def upload_group_document():
    try:
        ...
        unique_filename = f"document_{uuid.uuid4()}_{filename}"
        ...
        result = group_messages_collection.insert_one(document_message)
        document_id = str(result.inserted_id)

        # Emit socket event
        document_url = f'/get-group-document/{document_id}'
        ...
        socketio.emit('receive_group_message', {
            ...
            'type': 'document',
            'filename': filename,
            'timestamp': document_message['timestamp'].isoformat(),
            'read_by': [sender_id],
            'temp_id': temp_id
        }, room=group_id)
...
@app.route('/get-group-document/<document_id>', methods=['GET'])
def get_group_document(document_id):
    try:
        document_data = group_messages_collection.find_one({'_id': ObjectId(document_id)})
        if not document_data or 'file_path' not in document_data:
        ...
        return send_file(
            file_path, 
            as_attachment=True,
            download_name=document_data['filename']
        )
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def get_video(video_id):
    try:
        message = messages_collection.find_one({"_id": ObjectId(video_id), "is_video": True})
        if not message or not message.get('video_path'):
            return jsonify({"error": "Video not found"}), 404

        return send_file(message['video_path'], as_attachment=True, download_name=message['filename'])
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def handle_delete_group_message(data):
    try:
        ...
        if not all([message_id, group_id, action, user_id]):
